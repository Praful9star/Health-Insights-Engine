  ...
  if (user) return (
    <div className="relative z-10 min-h-[80vh] flex items-center justify-center px-4">
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="glass-panel rounded-3xl p-10 text-center max-w-sm w-full">
        ...
        <h2 className="text-xl font-serif font-800 text-foreground mb-1">Signed in</h2>
  ...
  if (done) return (
    <div className="relative z-10 min-h-[80vh] flex items-center justify-center px-4">
      <div className="glass-panel rounded-3xl p-10 text-center max-w-sm w-full">
        ...
        <h2 className="text-xl font-serif font-800 text-foreground mb-2">Check your email</h2>
        <p className="text-sm text-muted-foreground mb-6">Confirmation sent to <strong>{email}</strong>.</p>
        <Button className="w-full rounded-xl" onClick={() => { setDone(false); setMode("login"); }}>Back to sign in</Button>
      </div>
    </div>
  );
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.45 }} className="w-full max-w-sm">
        ...
        <div className="glass-panel rounded-3xl p-8 border border-border/60">
          <div className="flex items-center gap-3 mb-7">
            ...
            <div>
              <h1 className="text-xl font-serif font-800 text-foreground">{mode === "login" ? "Welcome back" : "Create account"}</h1>
              <p className="text-xs text-muted-foreground mt-0.5">{mode === "login" ? "Sign in to sync your health data" : "Free · no credit card needed"}</p>
            </div>
          </div>
          ...
          <form onSubmit={submit} className="space-y-3">
            {mode === "signup" && (
              <div className="relative">
                ...
                <Input placeholder="Full name" value={name} onChange={e => setName(e.target.value)} className="pl-9 rounded-xl" required />
              </div>
            )}
            <div className="relative">
              <Mail className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-muted-foreground" />
              <Input type="email" placeholder="Email" value={email} onChange={e => setEmail(e.target.value)} className="pl-9 rounded-xl" required />
            </div>
            <div className="relative">
              <Lock className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-muted-foreground" />
              <Input type="password" placeholder="Password" value={password} onChange={e => setPassword(e.target.value)} className="pl-9 rounded-xl" required minLength={6} />
            </div>
